...
def plot(r, u, obs_c, obs_r):
  r_x, r_y, r_z = r.swapaxes(0, 1)
  u_x, u_y, u_z = u.swapaxes(0, 1)
  obs_c_x, obs_c_y, obs_c_z = obs_c.swapaxes(0, 1)
  ...
  ax.plot(0.0, 0.0, 0.0, 'o', linestyle='', color='green', label='start')
  ax.plot(10.0, 10.0, 10.0, 'o', linestyle='', color='purple', label='dest')

  # plot obstacles
  for i in range(len(obs_c)):
    draw_sphere(ax, obs_c_x[i], obs_c_y[i], obs_c_z[i], obs_r[i])
